...
rel(home,	(x1 + d1 <= x2) || 
	...
	By stopping the search after one feasible solution is found the algorithm the search can often be accelerated. Although this slightly modified version is not complete anymore, in practice it is in some areas sufficient to know just one valid solution.
	...
	\begin{algorithm}
		\begin{algorithmic}
			...
			\EndFunction
		\end{algorithmic}
		\caption{Probe for Limited Discrepancy Search for a binary search tree from \cite{limitedDiscrepancy}}
	\end{algorithm}
	...
	\begin{algorithm}
		\begin{algorithmic}			
			...
			\EndFunction
		\end{algorithmic}
		\caption{Limited Discrepancy Search for a binary search tree from \cite{limitedDiscrepancy}}
	\end{algorithm}
