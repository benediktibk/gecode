...
\begin{document}
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\draw[very thick, fill=blue, fill opacity=0.1] (a) -- (b) -- (c) -- (d) -- (e) -- (a);
			\draw[pattern=north west lines, pattern color=gray] (a) -- (b) -- (c) -- (e) -- (a);
			\draw[pattern=north east lines, pattern color=gray] (f) -- (g) -- (c) -- (b) -- (f);
		\end{tikzpicture}	
		\caption{Branch-and-reduce applied to a search space}
		\label{fig:branchAndReduce}
	\end{figure}
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\node at (3.5, 1.5) {$P_2$};
		\end{tikzpicture}	
		\caption{Possible candidates for a solution and the related search space}
		\label{fig:bounding}
	\end{figure}
	
	Consider for this \reffig{fig:bounding}, where already a certain optimum $f_1$ was found in subproblem $P_1$. If now the relaxed optimum $f_{P0}$ is worse than this optimum
	\begin{equation}
		f_{P0} > f_1
	\end{equation}
	the whole subproblem $P_{0}$ can be pruned and it is not necessary to enumerate the solution candidates in it (or to apply split it further up).
